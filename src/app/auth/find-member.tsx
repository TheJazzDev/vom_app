...
export default function ActivateAccountScreen() {
  ...
  const { error, clearError, clearFoundMember, isfindingMemberForActivation } =
    useAuthSlice();
      ...
      if (findMemberForActivationThunk.fulfilled.match(result)) {
        if (result.payload) {
          router.push({
            ...
            params: {
              emailOrPhone: toInternationNigeriaPhone(data.emailOrPhone),
              authType: isEmail(data.emailOrPhone) ? 'email' : 'phone',
            },
          });
        }
      }
