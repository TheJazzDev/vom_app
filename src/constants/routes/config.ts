...
export type RouteAccess = 'public' | 'protected' | 'unprotected';
...
export const ROUTE_CONFIGS: RouteConfig[] = [
  ...
  },
  {
    ...
    fallbackRoute: ROUTES.AUTH,
    showUnauthorized: true,
    description: 'Past programmes - members only',
  },
  {
    path: ROUTES.MEMBERS,
    access: 'protected',
  ...
  // unprotected routes (unauthenticated users only)
  {
    path: ROUTES.AUTH,
    access: 'unprotected',
...
export function isPublicRoute(path: string): boolean {
  ...
  return config?.access === 'public' || !config;
}

export function isProtected(path: string): boolean {
  const config = getRouteConfig(path);
  return config?.access === 'protected';
}

export function isUnprotected(path: string): boolean {
  const config = getRouteConfig(path);
  return config?.access === 'unprotected';
}
